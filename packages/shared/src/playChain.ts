...
function isReturnTouchdown(play: Pick<PlaylistData, "spotEncoding">): boolean {
  ...
  if (!c) return false;
  // Scrimmage TDs are tackle:LOS|end:TD — not a return.
...
export function advanceSituation(play: PlayChainInput): SituationFields {
  ...
  if (play.result === Result.Fumble) {
    ...
    if (fumble) {
      ...
      const gain =
        play.odk === ODK.Defense ? -taggedGain : taggedGain;
...
export function nextDraftAfterPlay(
  ...
  if (afterKickoff) {
    ...
    return {
      ...
      yardLine:
        weKicked && situation.yardLine < 0
          ? flipHudlYardLinePerspective(situation.yardLine)
          : situation.yardLine,
      odk: weKicked ? ODK.Defense : ODK.Offense,
      ...emptyPlayers,
    };
  }
